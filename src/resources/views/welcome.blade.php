    ...
    <div class="w-full p-0">
        <main role="main" class="w-full flex flex-col h-screen content-center justify-center">
            <div class="mx-auto flex px-5 py-24 md:flex-row flex-col items-center h-full background-its">
                <div class="lg:flex-grow md:w-1/2 lg:pr-24 md:pr-16 flex flex-col md:items-start md:text-left mb-16 md:mb-0 items-center text-center mx-12">
                    ...
                    <div class="flex justify-center">
                        <a class="" href="{{ route('reservasi') }}">
                            <button class="inline-flex text-white bg-blue-400 shadow-xl border-0 py-4 px-6 mx-4 my-4 focus:outline-none hover:bg-yellow-500 rounded text-lg duration-200">Reservasi Sekarang <img style="width: 25px;" src="/pictures/icon/rightArrow.png" alt="">
                            </button>
                        </a>
                    </div>
                </div>
    ...
    <div class="relative h-full bg-white shadow-md sm:rounded-lg js-show-on-scroll tx-poppins px-6">
        <nav class="flex items-center justify-center flex-wrap w-full z-0 top-0 sticky sm:justify-between">
            <div class="flex items-center flex-shrink-0 text-white mt-6 ml-6">
                <div>
                    <span class="text-black font-bold no-underline text-2xl pl-2"><i class="em em-grinning"></i>Kontrol Ruangan</span>
                </div>
            </div>
        </nav>
        <section class="text-gray-600 body-font">
                            ...
                            <div class="xl:w-1/3 md:w-1/2 p-4">
                                <div class="border-2 border-yellow-400 px-6 py-12 rounded-lg bg-white shadow-2xl">
                                    ...
                                    <div class="flex flex-col justify-center">
                                        <h4 class="text-center my-2 font-medium">People in Room: <span id="persons{{ $rooms }}"></span></h4>
                                    </div>
                                    <div class="flex justify-center">
                                        @php($isEmpty = 1)
            ...
            <div class="js-show-on-scroll">
                ...
                <div class="container-fluid bg-yellow-4	00 p-10 text-center text-white">
...
<script>
    window.onload = function() {
        ...
        videos.forEach(function(video) {
            ...
            var imgId = 'img' + videoId.replace('video', '');
            checkVideoSource(video, imgId);
        });
    };
    ...
    function handleVideo(video) {
        ...
        setInterval(function() {
            if (video.readyState === video.HAVE_ENOUGH_DATA) {
                var screenshot = captureScreenshot(video);
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                $.ajax({
                    ...
                    dataType: 'JSON',
                    success: function(data) {
                        console.log('Prediction data:', data); // Debugging output
                        $('#persons' + videoId.replace('video', '')).html(data.person);
                    }
                });
            },
    ...
    function checkVideoSource(video, imgId) {
        var xhr = new XMLHttpRequest();
        xhr.open('HEAD', video.src, true);
        xhr.onload = function() {
            if (xhr.status === 200) {
                console.log('Video source valid:', video.src); // Debugging output
                video.style.display = 'block';
                document.getElementById(imgId).style.display = 'none';
                handleVideo(video);
            } else {
                console.log('Video source invalid:', video.src); // Debugging output
                video.style.display = 'none';
                document.getElementById(imgId).style.display = 'block';
            }
        };
        xhr.onerror = function() {
            console.log('Error checking video source:', video.src); // Debugging output
            video.style.display = 'none';
            document.getElementById(imgId).style.display = 'block';
        };
        xhr.send();
